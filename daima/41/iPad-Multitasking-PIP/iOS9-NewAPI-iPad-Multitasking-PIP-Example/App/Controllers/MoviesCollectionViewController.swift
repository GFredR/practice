...
extension MoviesCollectionDataSource {
    ...
    override func collectionView(_ collectionView: UICollectionView, cellForItemAt indexPath: IndexPath) -> UICollectionViewCell {
        ...
        guard let movie : MoviesLibrary = dataArray[(indexPath as NSIndexPath).item] as MoviesLibrary else { print("no value") }
        // Configure the cell
...
extension MoviesNavigation {
    ...
    override func prepare(for segue: UIStoryboardSegue, sender: AnyObject?) {
        if (segue.identifier == segueIdentifier) {
            if let row = (self.collectionView?.indexPathsForSelectedItems()?.first as NSIndexPath?)?.row, let detailViewController = segue.destinationViewController as? MovieDetailViewController {
