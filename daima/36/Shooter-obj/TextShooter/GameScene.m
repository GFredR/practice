...
- (instancetype)initWithSize:(CGSize)size levelNumber:(NSUInteger)levelNumber {
    if (self = [super initWithSize:size]) {
        ...
        SKLabelNode *lives = [SKLabelNode labelNodeWithFontNamed:@"gxj"];
        lives.fontSize = 16;
        lives.fontColor = [SKColor blackColor];
        lives.name = @"生命值";
        lives.text = [NSString stringWithFormat:@"生命: %lu",
        ...
        SKLabelNode *level = [SKLabelNode labelNodeWithFontNamed:@"gxj"];
        level.fontSize = 16;
        level.fontColor = [SKColor blackColor];
        level.name = @"生命值";
        level.text = [NSString stringWithFormat:@"级别: %lu",
...
//通过触摸移动处理方法touchesBegan设置利用屏幕底部五分之一的区域中的任意位置作为新的位置目标。
...
- (void)goToNextLevel {
    ...
    SKLabelNode *label = [SKLabelNode labelNodeWithFontNamed:@"gxj"];
    label.text = @"完成任务!";
...
- (void)setPlayerLives:(NSUInteger)playerLives  {
    ...
    SKLabelNode *lives = (id)[self childNodeWithName:@"生命值"];
    lives.text = [NSString stringWithFormat:@"生命: %lu",
                  (unsigned long)_playerLives];
}
...
- (void)createForceFields {
    ...
    for (NSUInteger i = 0; i < fieldCount; i++) {
        ...
        SKLabelNode *fieldLocationNode = [SKLabelNode labelNodeWithFontNamed:@"gxj"];
